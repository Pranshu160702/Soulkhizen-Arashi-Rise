...
namespace EpicTransport {
    public abstract class Common {
        ...
        protected void Dispose() {
            ...
            if (p2p != null)
            {
                p2p.RemoveNotifyPeerConnectionRequest(incomingNotificationId);
                p2p.RemoveNotifyPeerConnectionClosed(outgoingNotificationId);
            }
        ...
        protected void SendInternal(ProductUserId target, SocketId socketId, InternalMessages type) {
            var options = new SendPacketOptions() {
                ...
                Data = new System.ArraySegment<byte>(new byte[] { (byte) type }),
        ...
        protected void Send(ProductUserId host, SocketId socketId, byte[] msgBuffer, byte channel) {
            var options = new SendPacketOptions() {
                AllowDelayedDelivery = true,
                Channel = channel,
                Data = new System.ArraySegment<byte>(msgBuffer),
                LocalUserId = EOSSDKComponent.LocalUserProductId,
        ...
        private bool Receive(out ProductUserId clientProductUserId, out SocketId socketId, out byte[] receiveBuffer, byte channel) {
            ...
            // Guard against null instance during shutdown
            if (EOSSDKComponent.LocalUserProductId == null) return false;
            ...
            ProductUserId tempUserId = null;
            SocketId tempSocketId = default;
            byte outChannel;
            ArraySegment<byte> dataSegment = new ArraySegment<byte>(new byte[1170]);
            uint bytesWritten;
            Result result = EOSSDKComponent.GetP2PInterface().ReceivePacket(ref options, ref tempUserId, ref tempSocketId, out outChannel, dataSegment, out bytesWritten);
            if (result == Result.Success) {
                clientProductUserId = tempUserId;
                socketId = tempSocketId;
                receiveBuffer = new byte[bytesWritten];
                Array.Copy(dataSegment.Array, receiveBuffer, bytesWritten);
                return true;
            }
            return false;
        }
