...
namespace EpicTransport {
    ...
    public class EOSSDKComponent : MonoBehaviour {
        ...
        public static bool CollectPlayerMetrics {
            get {
                if (instance == null || applicationIsQuitting) return false;
        ...
        public static Epic.OnlineServices.Metrics.MetricsInterface GetMetricsInterface() => Instance?.EOS?.GetMetricsInterface();
        ...
        public static bool Initialized {
            get {
                return instance != null && !applicationIsQuitting && instance.initialized;
            }
        }
        ...
        private void Awake() {
            Debug.Log("[EOSSDKComponent] Awake");
...
#if UNITY_EDITOR
            var libraryPath = "Packages/com.playeveryware.eos/Runtime/Windows/x64/EOSSDK-Win64-Shipping.dll";
        ...
        protected void InitializeImplementation() {
            ...
            var initializeOptions = new InitializeOptions() {
                ProductName = apiKeys != null ? apiKeys.epicProductName : "INDG",
                ProductVersion = apiKeys != null ? apiKeys.epicProductVersion : "1.0"
            };
            ...
            // AlreadyConfigured is fine in builds too (not just editor) when re-entering
            var isAlreadyConfigured = initializeResult == Result.AlreadyConfigured;
            if (initializeResult != Result.Success && !isAlreadyConfigured) {
            ...
            LogMessageFunc logCallback = (ref LogMessage message) => Logger.EpicDebugLog(message);
            ...
            Debug.Log("[EOSSDKComponent] PlatformInterface created successfully");
            ...
            if (authInterfaceLogin) {
                ...
                EOS.GetAuthInterface().Login(ref loginOptions, null, (ref Epic.OnlineServices.Auth.LoginCallbackInfo info) => OnAuthInterfaceLogin(info));
            } else {
                ...
                if (connectInterfaceCredentialType == Epic.OnlineServices.ExternalCredentialType.DeviceidAccessToken) {
                    // Delete existing device ID first so we can create a unique one per instance
                    var deleteOptions = new Epic.OnlineServices.Connect.DeleteDeviceIdOptions();
                    EOS.GetConnectInterface().DeleteDeviceId(ref deleteOptions, null, (ref Epic.OnlineServices.Connect.DeleteDeviceIdCallbackInfo deleteInfo) =>
                    {
                        Debug.Log($"[EOSSDKComponent] DeleteDeviceId result: {deleteInfo.ResultCode}");
        ...
        public static void Initialize() {
            if (instance == null || applicationIsQuitting) return;
            if (Instance.initialized || Instance.isConnecting) {
                return;
            }
            if (Instance.apiKeys == null) {
                Debug.LogWarning("[EOSSDKComponent] apiKeys not assigned, skipping initialize");
                return;
            }
        ...
        private void CreateUniqueDeviceId() {
            ...
            #endif
            Debug.Log($"[EOSSDKComponent] Creating device ID with model: {uniqueModel}");
            var createDeviceIdOptions = new Epic.OnlineServices.Connect.CreateDeviceIdOptions();
            createDeviceIdOptions.DeviceModel = uniqueModel;
            EOS.GetConnectInterface().CreateDeviceId(ref createDeviceIdOptions, null, (ref Epic.OnlineServices.Connect.CreateDeviceIdCallbackInfo info) => OnCreateDeviceId(info));
        }
        ...
        private void ConnectInterfaceLogin() {
            ...
            if (connectInterfaceCredentialType == Epic.OnlineServices.ExternalCredentialType.Epic) {
                ...
                var copyTokenOptions = new Epic.OnlineServices.Auth.CopyUserAuthTokenOptions();
                Result result = EOS.GetAuthInterface().CopyUserAuthToken(ref copyTokenOptions, localUserAccountId, out token);
                ...
                loginOptions.UserLoginInfo = new Epic.OnlineServices.Connect.UserLoginInfo() { DisplayName = displayName };
            }
            ...
            EOS.GetConnectInterface().Login(ref loginOptions, null, (ref Epic.OnlineServices.Connect.LoginCallbackInfo info) => OnConnectInterfaceLogin(info));
        }
        ...
        private void OnConnectInterfaceLogin(Epic.OnlineServices.Connect.LoginCallbackInfo loginCallbackInfo) {
            if (loginCallbackInfo.ResultCode == Result.Success) {
                ...
                initialized = true;
                isConnecting = false;
                Debug.Log($"[EOSSDKComponent] EOS fully initialized. ProductId: {localUserProductIdString}");

                var authExpirationOptions = new Epic.OnlineServices.Connect.AddNotifyAuthExpirationOptions();
                authExpirationHandle = EOS.GetConnectInterface().AddNotifyAuthExpiration(ref authExpirationOptions, null, (ref Epic.OnlineServices.Connect.AuthExpirationCallbackInfo info) => OnAuthExpiration(info));
            } else if (Epic.OnlineServices.Common.IsOperationComplete(loginCallbackInfo.ResultCode)) {
                Debug.Log("Login returned " + loginCallbackInfo.ResultCode + "\nRetrying...");
                var createUserOptions = new Epic.OnlineServices.Connect.CreateUserOptions() { ContinuanceToken = loginCallbackInfo.ContinuanceToken };
